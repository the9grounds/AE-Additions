...
class ExtraCellsCellInventory<T: IAEStack<T>?>(cellType: IExtraCellsStorageCell<T>, itemStack: ItemStack, container: ISaveProvider?) : AbstractExtraCellsInventory<T>(cellType, itemStack, container) {
...
//        }
    ...
    companion object {
        ...
        private fun getStorageCell(itemStack: ItemStack?): IExtraCellsStorageCell<*>? {
            if (itemStack != null) {
                val type = itemStack.item

                if (type is IExtraCellsStorageCell<*>) {
                    return type
                }
            }
        ...
        @JvmStatic fun <T : IAEStack<T>?> createInventory(itemStack: ItemStack?, container: ISaveProvider?): ICellInventory<T>? {
            try {
                ...
                val type = itemStack.item
                val cellType: IExtraCellsStorageCell<T>

                if (type is IExtraCellsStorageCell<*>) {
                    cellType = type as IExtraCellsStorageCell<T>
                } else {
                    throw AppEngException("ItemStack was used as a cell, but was not a cell!")
                }
                ...
                return ExtraCellsCellInventory(cellType, itemStack, container)
